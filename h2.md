...
## Setting up
I started off with the command:
```bash
$ mkdir terosChallenge && cd terosChallenge && wget https://terokarvinen.com/hack-n-fix/teros-challenges.zip
```
This created a directory to store all the files, moves into it and downloads the necessities.     
We can then unzip the contents:
```
$ unzip teros-challenges.zip
```
<img width="1227" height="674" alt="Screenshot from 2026-01-22 17-59-43" src="https://github.com/user-attachments/assets/c7cabac1-32d5-4f10-8385-11991bc0fc2c" />     
...
The website is kind enough to let us know the SQL query used to recover it, which is `SELECT password FROM pins WHERE pin='1';`.      
So I tried to inject it by typing `' OR 1=1--` into the field:
