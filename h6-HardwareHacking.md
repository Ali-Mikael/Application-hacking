...
I don't want to focus on debugging this for hours on end, so I messaged a [friend](<https://github.com/MatPohj>), who also had the same issue as me.

ChatGPT confirmed this also.

The problem is the code syntax is outdated, so we have to add the `-std=gnu89` flag to the makefile like this:
```bash
$ vim Makefile
>>>
...
To tackle this, we download the necessary dependency (which I thought the `preinstall` script would've taken care of)
```bash
